...
class TransitionProbabilities:
    ...
    def m_A_sq(self, z, omega, x_e=None):
        ...
        m_A_sq = 1.4e-21 * (x_e - 6e-3 * (omega / eV) ** 2 * (1 - x_e)) * (
            self.n_H(z) / Centimeter ** -3)
    ...
    def n_H(self, z):
        ...
        """
        return (1 - self.Y_p) * self.eta * 2 * self.xi_3 / np.pi ** 2 * (self.T_0_n * (1 + z)) ** 3
    ...
    def get_z_crossings(self, m_A, omega_0, evolve_z=True):
        ...
        where_ary = np.where(np.logical_or(
            (m_A_ary[:-1] < m_A) * (m_A_ary[1:] > m_A), (m_A_ary[:-1] > m_A) * (m_A_ary[1:] < m_A)))
        ...
        for i in range(len(where_ary[0])):
            z_cross_ary.append(
                brentq(m_A_sq, z_ary[where_ary[0][i]], z_ary[where_ary[0][i] + 1]))
    ...
    def P_tot(self, omega_0, eps, m_A, approx_linearize=True, evolve_z=True, sum_probs=False, **kwargs):
        ...
        z_res_ary = [self.get_z_crossings(
            m_A, omega, evolve_z) for omega in omega_0]

        # Get transition probabilities at resonance

        if sum_probs:
            P_ary = np.array([np.nansum(self.P_trans(m_A, z, omega, eps, approx_linearize=approx_linearize,
                                                     evolve_z=evolve_z)) for z, omega in zip(z_res_ary, omega_0)], dtype=object)
        else:
            P_ary = np.array([(self.P_trans(m_A, z, omega, eps, approx_linearize=approx_linearize,
                                            evolve_z=evolve_z)) for z, omega in zip(z_res_ary, omega_0)], dtype=object)
...
class PerturbedProbability(TransitionProbabilities):
    ...
    def __init__(self, log_pk_b_interp_fn, log_pk_e_interp_fn=None, log_pk_b_e_interp_fn=None, **kwargs):
        TransitionProbabilities.__init__(self, **kwargs)
        self.log_pk_b_interp_fn = log_pk_b_interp_fn
        self.log_pk_e_interp_fn = log_pk_e_interp_fn
        self.log_pk_b_e_interp_fn = log_pk_b_e_interp_fn
    ...
    def _delta_sq(self, z_ary, k_min, k_max, log_pk_interp_fn, r_smooth=None):
        ...
        """
        k_ary = np.logspace(np.log10(k_min), np.log10(k_max), 1000)
        integrand_ary = (k_ary ** 2 / (2 * np.pi ** 2)) * \
            10 ** log_pk_interp_fn(z_ary, k_ary)
    ...
    def _xi(self, z_ary, r, k_min, k_max, log_pk_interp_fn, r_smooth=None):
        """ Get correlation function of a power spectrum between scales k_min and k_max over redshifts z_ary. Returns matrix over redshifts.
        """
        k_ary = np.logspace(np.log10(k_min), np.log10(k_max), 100)
        integrand_ary = (k_ary ** 2 / (2 * np.pi ** 2)) * \
            10 ** log_pk_interp_fn(z_ary, k_ary) * \
            np.sin(k_ary * r) / (k_ary * r)
    ...
    def _dP_dz(self, z_ary, m_Ap, k_min, k_max, omega, pdf='lognormal', one_plus_delta_bound=None, b=1, Ap_DM=False, x_e_ary=None, eng_weight=False, r_smooth=None, return_pdf=False, pspecs_mode='chi'):
        ...
        if self.log_pk_e_interp_fn is None and self.log_pk_b_e_interp_fn is None:
            delta_b_b = delta_e_e = delta_e_b = self._delta_sq(
                z_ary, k_min, k_max, self.log_pk_b_interp_fn, r_smooth=r_smooth)
        else:
            delta_b_b = self._delta_sq(
                z_ary, k_min, k_max, self.log_pk_b_interp_fn, r_smooth=r_smooth)
            delta_e_e = self._delta_sq(
                z_ary, k_min, k_max, self.log_pk_e_interp_fn, r_smooth=r_smooth)
            delta_e_b = self._delta_sq(
                z_ary, k_min, k_max, self.log_pk_b_e_interp_fn, r_smooth=r_smooth)

        z_thresh = self.z_reio + 2.

        delta_e_e[z_ary < z_thresh] = delta_b_b[z_ary < z_thresh]
        delta_e_b[z_ary < z_thresh] = delta_b_b[z_ary < z_thresh]

        n_H_ary = self.n_H(z_mesh) / (Centimeter ** -3)
        A = 1.4e-21 * eV ** 2
        B = 1.02e-23

        sigma_sq_ary = n_H_ary ** 2 * ((A + B * omega_res_mesh ** 2) ** 2 * x_e_ary ** 2 * delta_e_e - 2 * (A + B * omega_res_mesh ** 2) * B * omega_res_mesh ** 2 * x_e_ary * (
            1 - x_e_ary) * delta_e_b + B ** 2 * omega_res_mesh ** 4 * delta_b_b * (1 - x_e_ary) ** 2)
        ...
        if pdf == 'lognormal':
            ...
            pdf_ary = 1. / np.sqrt(2 * np.pi * sigma_sq_ary) * \
                np.exp(-(m_A_sq - m_Ap ** 2) ** 2 / (2 * sigma_sq_ary))

            dP_dz_ary = m_Ap ** 2 / np.sqrt(2 * np.pi * sigma_sq_ary) * np.exp(-(
                m_A_sq - m_Ap ** 2) ** 2 / (2 * sigma_sq_ary)) / np.abs(self.dz_dt(z_mesh))
            ...
            z_void_data_ary = np.array(
                [0., 0.3, 0.6, 1., 1.5, 2.2, 3., 4., 5.3, 6.9, 9., 12])
            ...
            t_ary = (np.log10(one_plus_delta_ary*Omega_m) -
                     mu_void_ary) / sigma_void_ary

            voids_pdf = voids_vol_frac / np.sqrt(2*np.pi)*np.exp(-t_ary**2/2) * (1. + erf(
                alpha_void_ary*t_ary/np.sqrt(2))) / sigma_void_ary / one_plus_delta_ary / np.log(10)

            pdf_ary = voids_pdf / m_A_sq

            dP_dz_ary = voids_pdf / \
                np.abs(self.dz_dt(z_mesh)) * m_Ap**2 / m_A_sq
            ...
            dP_dz_ary = (10 ** log_pdf) / \
                np.abs(self.dz_dt(z_mesh)) * m_Ap ** 2 / m_A_sq

            dP_dz_ary *= weight

            # Switch to Gaussian above z = 150.

            pdf_gauss_ary = 1. / np.sqrt(2 * np.pi * sigma_sq_ary) * \
                np.exp(-(m_A_sq - m_Ap ** 2) ** 2 / (2 * sigma_sq_ary))

            dP_dz_gauss_ary = m_Ap ** 2 / np.sqrt(2 * np.pi * sigma_sq_ary) * np.exp(-(
                m_A_sq - m_Ap ** 2) ** 2 / (2 * sigma_sq_ary)) / np.abs(self.dz_dt(z_mesh))

            dP_dz_gauss_ary *= weight

            pdf_ary[:, z_ary > 150.] = pdf_gauss_ary[:, z_ary > 150.]

            dP_dz_ary[:, z_ary > 150.] = dP_dz_gauss_ary[:, z_ary > 150.]

        # Remove small and large perturbation regions corresponding to `one_plus_delta_bound`
        if one_plus_delta_bound is not None:

            if callable(one_plus_delta_bound):

                bound_low, bound_upp = one_plus_delta_bound(z_ary)

                restrict_index = (one_plus_delta_ary < bound_low) + \
                    (one_plus_delta_ary > bound_upp)

            else:

                restrict_index = (one_plus_delta_ary > one_plus_delta_bound) + \
                    (one_plus_delta_ary < (1 / one_plus_delta_bound))
    ...
    def P_tot_perturb(self, omega, eps, m_Ap, k_min=1e-3, k_max=1e4, z_min=1e-3, z_max=1.5e2, z_excise_min=None, z_excise_max=None, z_int=None, pdf='lognormal', one_plus_delta_bound=None, b=1., Ap_DM=False, eng_weight=False, r_smooth=None, n_z_bins=10000,
        ...
        :param pdf: Distribution of perturbations. 'gaussian', 'lognormal', 'voids', or a `RegularGridInterpolator` instance for a custom pdf.
        :param one_plus_delta_bound: If float, restrict to fluctuations (1 + \delta) times and divided by this quantity. If callable, takes a function that returns (low,upp) limits given z.
        :param b: Bias parameter for lognormal pdf
        :param Ap_DM: Dark photon dark matter case
        :param eng_weight: Weight dP/dz by m_Ap**2/m_A_sq, for dark photon -> photon energy injection case. 
        :param r_smooth: Smoothing scale, by default no smoothing
        ...
        z_cross_ary, _, P_tot_ary = self.P_tot(
            omega, eps, m_Ap, evolve_z=evolve_z)

        # Only keep homogeneous transitions above z_max
        P_tot_homo_list = [P_tot[z_cross > z_max]
                           for P_tot, z_cross in zip(P_tot_ary, z_cross_ary)]

        z_cross_homo_list = [z_cross[z_cross > z_max]
                             for z_cross in z_cross_ary]

        # Sum the homogeneous probabilities above z_max that we kept
        P_tot_homo_ary = np.array([np.sum(P_tot[z_cross > z_max])
                                   for P_tot, z_cross in zip(P_tot_ary, z_cross_ary)])
        ...
        if z_int is not None:
            ...
            P_tot_homo_ary = np.array([np.sum(P_tot[(z_cross > z_int[0]) & (
                z_cross < z_int[1])]) for P_tot, z_cross in zip(P_tot_homo_list, z_cross_homo_list)])

        # Get differential probability
        dP_dz_ary, sigma_sq_ary = self._dP_dz(
            z_ary, m_Ap, k_min, k_max, omega, pdf, one_plus_delta_bound, b, Ap_DM=Ap_DM, eng_weight=eng_weight, r_smooth=r_smooth)
        ...
        if z_excise_min is not None and z_excise_max is not None:
            ...
            dP_dz_excised_low_ary = np.split(np.extract(
                np.tile(z_ary < z_excise_min, (len(omega), 1)), dP_dz_ary), len(omega))
            dP_dz_excised_high_ary = np.split(np.extract(
                np.tile(z_ary > z_excise_max, (len(omega), 1)), dP_dz_ary), len(omega))

            P_tot = np.trapz(np.nan_to_num(dP_dz_excised_low_ary), z_excised_low_ary) + \
                np.trapz(np.nan_to_num(dP_dz_excised_high_ary),
                         z_excised_high_ary)
        ...
        # If we are interested in particulars of higher-z homogeneous crossings
        if get_total_prob:
            P_tot_homo_nosum_ary = np.array([np.array(
                P_tot[z_cross > z_max]) for P_tot, z_cross in zip(P_tot_ary, z_cross_ary)])
            z_cross_homo_nosum_ary = np.array([np.array(
                z_cross[z_cross > z_max]) for P_tot, z_cross in zip(P_tot_ary, z_cross_ary)])
            return z_ary, dP_dz_ary, P_tot, sigma_sq_ary, z_cross_homo_nosum_ary, P_tot_homo_nosum_ary
...
class FIRAS(PerturbedProbability):
    ...
    def __init__(self, log_pk_b_interp_fn, log_pk_e_interp_fn=None, log_pk_b_e_interp_fn=None, PIXIE=False, **kwargs):
        PerturbedProbability.__init__(
            self, log_pk_b_interp_fn, log_pk_e_interp_fn, log_pk_b_e_interp_fn, **kwargs)
        self.eps_base = 1e-7
        self.PIXIE = PIXIE
    ...
    def set_up_firas(self):
        ...
        if not os.path.isfile('../data/firas_monopole_spec_v1.txt'):
            ...
            urllib.request.urlretrieve(
                url_spec, '../data/firas_monopole_spec_v1.txt')
        ...
        if not self.PIXIE:
            ...
            for i1 in range(len(df)):
                for i2 in range(len(df)):
                    ...
                    idx_Q = (np.abs(nu_minus_nu_prime_ary -
                                    nu_minus_nu_prime)).argmin()
                    Q = Q_ary[idx_Q]
    ...
    def chi2_FIRAS(self, x, eps, P_tot):
        ...
        """
        T_0 = x[0]
        t = self.B_CMB(self.omega_FIRAS, T_0) * (1 - P_tot *
                                                 (eps / self.eps_base) ** 2) / (1e6 * Jy)
        return (np.dot((self.d - t), np.matmul(self.Cinv, (self.d - t))))

    def chi2_FIRAS_P(self, x, P_tot):
        """ The FIRAS \chi^2 statistic, to do minimization over blackbody temperature
        """
        T_0 = x[0]
        t = self.B_CMB(self.omega_FIRAS, T_0) * (1 - P_tot) / (1e6 * Jy)
        return (np.dot((self.d - t), np.matmul(self.Cinv, (self.d - t))))
    ...
    def chi2_FIRAS_scan(self, m_A, homo=False, **kwargs):
        ...
        if homo:
            P_tot = self.P_tot(self.omega_FIRAS, self.eps_base,
                               m_A, sum_probs=1, **kwargs)[2]
        else:
            P_tot = self.P_tot_perturb(
                self.omega_FIRAS, self.eps_base, m_A, **kwargs)[2]

        # Null chi2
        chi2_null = minimize(self.chi2_FIRAS, x0=[2.725], args=(
            0, np.ones_like(self.omega_FIRAS)), method='SLSQP').fun
        ...
        for i_eps, eps in enumerate(self.eps_ary):
            chi2_min = minimize(self.chi2_FIRAS, x0=[
                                2.725], args=(eps, P_tot), method='SLSQP')
            chi2_ary[i_eps] = chi2_min.fun

        # Return \delta\chi^2
        return chi2_ary - chi2_null

    def get_lim(self, delta_chi2_ary, eps_ary=None):
        ...
        for i_m in range(len(delta_chi2_ary)):
            ...
            for xi in range(max_loc, len(eps_ary)):
                ...
                if val > self.delta_chi2_95_level_one_sided:
                    scale = (-TS_eps_ary[xi-1] + self.delta_chi2_95_level_one_sided) / \
                        (-TS_eps_ary[xi-1] + TS_eps_ary[xi])
                    lim_ary[i_m] = 10 ** (np.log10(eps_ary[xi - 1]) + scale * (
                        np.log10(eps_ary[xi]) - np.log10(eps_ary[xi - 1])))
                    break
